...
//                intent.putExtra("cols", applicableTestRoomLocation);
        ...
        for (int i = 1; i <= rows; i++) {
            ...
            for (int j = 1; j <= 1; j++) {
                if (i == 1 && j == 1) {
                    ...
                    layoutParams.width = getResources().getDimensionPixelSize(R.dimen.common_text_cell_width);
                    layoutParams.height = getResources().getDimensionPixelSize(R.dimen.common_txt_header_height);
            ...
            for (int j = 1; j <= 1; j++) {
                if (i == 1 && j == 1) {
                    ...
                    if (i == 2) {
                        ...
                        ViewGroup.LayoutParams layoutParams = etv.getLayoutParams();
                        layoutParams.width = getResources().getDimensionPixelSize(R.dimen.common_text_cell_width);
                        etv.setLayoutParams(layoutParams);
    ...
    private void addRowButtonClick(int rows) {
        ...
        ViewGroup.LayoutParams etlayoutParams = et.getLayoutParams();
        etlayoutParams.width = getResources().getDimensionPixelSize(R.dimen.common_text_cell_width);
            ...
            if (TestCreateActivity.ACPHAV.equals(testType)) {
                ...
                for (int i = 0; i < airFlowRateTxtViewList.size(); i++) {
                    ...
                    if (airFlowRateTxtViewList.get(i).getTag().equals(tagF)) {
                        ...
                        float aXAv=getMultiplicationAirFlow((int) tvl.getTag(), txtViewList.get(i).getText().toString());
                        tvl.setText(""+aXAv);
                        resultDataHashMap2.put(tvl.getId(),aXAv);
                ...
                if (viewById >= 900) {
                    for (int i = 0; i < editTextList.size(); i++) {
                        if (editTextList.get(i).getId() == (800 + (viewById - 900))) {
                            ...
                            // Pass Fail Calculation
                            TextView txtPassFail = txtPassFailList.get(i);
                            double absSlpDlpValue = Math.abs(slpDlpValue);
                            Log.d(TAG, "abs slpDlpValue=" + absSlpDlpValue + " mTolarence=" + mTolarence);
                            if (absSlpDlpValue <= mTolarence) {
                                ...
                                passFailHashMap.put(txtPassFail.getId(), " PASS ");
                            } else if (absSlpDlpValue > mTolarence) {
                                if (null != mFitPassFailFlagList && i >= mFitPassFailFlagList.size()) {
...
//            }
    ...
    /**
     * Calculating the difference percentage in FIT test
     *
